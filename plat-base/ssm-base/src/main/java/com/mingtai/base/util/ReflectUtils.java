...
public class ReflectUtils {
	 ...
	 * @return
	 */
	public static String getFeildPkFeildName (final Object object){
		Assert.notNull(object, "object不能为空");
		Method[] methods = getDeclaredMethods(object);
		for(Method m : methods){
			Annotation[] allAnnotations = m.getAnnotations();
			for(Annotation an : allAnnotations){
				Class a = an.annotationType();
				if(a.getName().indexOf("persistence.Id")!=-1){
					String s = m.getName();
					if(!StringUtils.isBlank(s)){
						return StringUtils.changeFirstCharacterCase(s.replace("get",""),false);
					}
				}
			}
		}
